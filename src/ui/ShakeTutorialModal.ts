...
export class ShakeTutorialModal {
  constructor(scene: Phaser.Scene) {
    ...
    const title = scene.add.text(494, 188, 'КОКТЕЙЛИ В ШЕЙКЕРЕ', { fontFamily: DISPLAY_FONT, fontSize: '20px', color: '#f3c96b' }).setOrigin(0.5)
    const methodIcon = createPreparationMethodIcon(scene, 'shake', 365, 188, 30)
    ...
    root.add([blocker, panel, methodIcon, title, text, button, label])
  }
}
