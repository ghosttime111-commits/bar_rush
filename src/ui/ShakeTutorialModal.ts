...
export class ShakeTutorialModal {
  constructor(scene: Phaser.Scene) {
    ...
    const title = scene.add.text(480, 188, '🧋  КОКТЕЙЛИ В ШЕЙКЕРЕ', { fontFamily: 'Trebuchet MS', fontSize: '22px', fontStyle: 'bold', color: '#f3c96b' }).setOrigin(0.5)
    const text = scene.add.text(480, 260, 'Если в рецепте указан шейкер:\n1. Добавьте ингредиенты по порядку.\n2. Нажмите «Взболтать».\n3. Подайте готовый коктейль.', {
      fontFamily: 'Trebuchet MS', fontSize: '14px', color: '#eee3e9', align: 'center', lineSpacing: 8,
    }).setOrigin(0.5)
    ...
    const label = scene.add.text(480, 350, 'ПОНЯТНО', { fontFamily: 'Trebuchet MS', fontSize: '13px', fontStyle: 'bold', color: '#fff' }).setOrigin(0.5)
